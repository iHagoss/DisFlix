...
class AuthRepository(private val context: Context) {
    ...
    suspend fun logout(): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (authKey != null) {
                    ApiClient.stremioApi.logout(LogoutRequest(authKey = authKey))
                }
                clearAuth()
                Result.success(Unit)
            } catch (e: Exception) {
    ...
    suspend fun refreshUser(): Result<User> {
        return withContext(Dispatchers.IO) {
            try {
                val authKey = getAuthKey() ?: return@withContext Result.failure(Exception("Not logged in"))
                
                val response = ApiClient.stremioApi.getUser(GetUserRequest(authKey = authKey))
